...
class Bot:
    """
    Бот для vk.com, запрашивающий у пользователя показания счетчиков и отправляющий их в управляющую компанию по
    ...
    def continue_scenario(self, user_state, text, user_subscribed):
        ...
        :param text: текст полученного сообщения
        :param user_subscribed: запись из БД c информацией по пользователю (имя, дата отправки показаний, номер
        квартиры, email, типы счетчиков)
        :return: текст сообщения, которое будет отправлено пользователю; содержит объяснение того, что ему нужно
